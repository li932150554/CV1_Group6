...
class Problem2:
    ...
    def find_matches(self, p1, p2, distances):
        ...
        # print(pairs)
        # print("shape of paris:", np.shape(pairs))
    ...
    def compute_homography_distance(self, H, p1, p2):
        ...
        # p1H = np.insert(p1, 2, values=a, axis=1)
        # p2H = np.insert(p2, 2, values=a, axis=1)
        # dist = np.norm(np.dot(H, p1H) - p2H)**2 + np.norm(p1H - np.dot(np.inv(H), p2H))**2
