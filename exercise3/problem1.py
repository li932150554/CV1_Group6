...
def compute_hessian(img, gauss, fx, fy):
    ...
    I_xx = convolve(convolve(img, fx, mode='mirror'), fx, mode='mirror')
    I_yy = convolve(convolve(img, fy, mode='mirror'), fy, mode='mirror')
    I_xy = convolve(convolve(img, fx, mode='mirror'), fy, mode='mirror')
...
def compute_criterion(I_xx, I_yy, I_xy, sigma):
    ...
    """
    criterion = (sigma**4) * (I_xx * I_yy - I_xy*I_xy) # np.multiply(I_xx, I_yy)
    return criterion
    #
    # You code here
    #
...
def nonmaxsuppression(criterion, threshold):
    ...
    """
    criterion_nms = maximum_filter(criterion, size=5)
    rows, cols = np.nonzero(criterion_nms > threshold)
    return rows, cols
    #
    # You code here
    #
